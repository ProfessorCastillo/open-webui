						...
						<Tooltip content={$i18n.t('Edit')} placement="bottom">
							<button
								...
								class="{($settings?.highContrastMode ?? false)
									? ''
									: 'opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus-visible:opacity-100'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition edit-user-message-button"
								on:click={() => {
									editMessageHandler();
								}}
							>
						...
						{#if !readOnly && (!isFirstMessage || siblings.length > 1)}
							<Tooltip content={$i18n.t('Delete')} placement="bottom">
								<button
									...
									class="{($settings?.highContrastMode ?? false)
										? ''
										: 'opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus-visible:opacity-100'} p-1 rounded-sm dark:hover:text-white hover:text-black transition"
									on:click={(e) => {
										if (e.shiftKey) {
											deleteMessageHandler();
										} else {
